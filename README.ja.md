...
`gh pr list` でオープンな PR を一覧表示し、fzf で選択したブランチの worktree を作成します。ローカルに存在しないリモートブランチも自動でフェッチしてトラッキング設定を行います。
...
### `gcreate` — リポジトリを作成してクローン

新しい GitHub リポジトリを作成し、`ghq` でクローンします。クローン後に `cd` するか確認プロンプトを表示します。デフォルトはプライベートリポジトリです。

```
gcreate [-h] [-p] [-P] [-d <説明>] [リポジトリ名]
```
...
| `-l`, `--license [id]` | ライセンスを追加 — `id` 省略時は fzf で選択 |
| `-g`, `--gitignore [tmpl]` | .gitignore を追加 — テンプレート省略時は fzf で選択 |
| `--disable-issues` | Issues を無効化 |
